...
10. **`apply-pick` post-check revert** → pre-stage a dead index row so `validate-wiki.js all` exits 2 on the post-check → script auto-reverts the commit; script exits 2; entry stays `unresolved`; yaml entry unchanged. (Broken wikilinks introduced by the rewrite would only trigger exit 1 — a warning per CR-005 conventions, not a revert trigger; lint surfaces those on the next pass.)
